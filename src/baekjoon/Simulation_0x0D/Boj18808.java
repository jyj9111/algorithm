...
public class Boj18808 { // Boj18808: 스티커 붙이기 (시뮬레이션)
        ...
        for (int stk = 0; stk < kNum; stk++) {
            ...
            while(num > 0) {
                if(md.compare(sticker)) {
                    ...
                    sticker = md.rotateSticker(sticker, num);
                }
                num--;
            }
        }
    ...
    private int[][] rotateSticker(int[][] sticker, int dir) {
    ...
    private void putSticker(int[][] sticker) {
        System.out.println(saveXY[0]);
        System.out.println(saveXY[1]);
